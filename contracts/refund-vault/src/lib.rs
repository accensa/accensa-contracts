...
/// Shared implementation of `__constructor` / `initialize` (issue #129).
///
/// Not auth-gated (like the pre-#129 `initialize`): factory deployment
/// authorizes the merchant on `deploy_vault` before the constructor runs, and
/// direct deployments make their own auth choice (e.g. a multisig account
/// signer set).
fn init_vault(env: &Env, init: VaultInit) -> Result<(), Error> {
    if env.storage().instance().has(&DataKey::Admin) {
        return Err(Error::AlreadyInitialized);
    }

    let window = init.refund_window;
    let deadline = init.deadline;
    let vdf_delay = init.vdf_delay;
    ...
    env.storage()
        .instance()
        .set(&DataKey::RefundDeadline, &deadline);
    env.storage().instance().set(&DataKey::VdfDelay, &vdf_delay);
    env.storage()
        .instance()
...
#[contractimpl]
impl RefundVault {
    /// Constructor used by `RefundVaultFactory::deploy_vault` (`deploy_v2`,
    /// issue #129). Records the merchant, token, the addresses of the
    /// stateless policy contracts, the initial gate mirrors, and the fee
    /// configuration from a single [`VaultInit`].
    pub fn __constructor(env: Env, init: VaultInit) -> Result<(), Error> {
        init_vault(&env, init)
    }

    /// `initialize` alias of [`__constructor`](Self::__constructor) for
    /// direct (non-factory) deployments.
