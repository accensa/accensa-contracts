...
| `UserNonce(Address)` | Persistent | `u64` | Small | Per-caller replay-protection counter (issue #122). The merchant's next `refund`/`claim_batch`/`process_batch` call must present this value as its `nonce`; the call reverts with `StaleState` on a mismatch and the value increments only on success. Absent = `0`. |
| `Nonce` | Instance | `u64` | Small | Global monotonic counter used as a domain separator / per-operation nonce for multi-call coordination (issue #136); distinct from the per-user replay nonce. |
